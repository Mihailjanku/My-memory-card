...
def show_question():
    ...
    RadioGroup.setExclusive(
        False
    )  # removed the restrictions so as to reset the radio button choice
    ...
    RadioGroup.setExclusive(
        True
    )  # returned the restrictions, now only one radio button can be selected
...
def ask(q: Question):
    ...
    shuffle(
        answers
    )  # shuffled the list of buttons, now some random button is first in the list
    answers[0].setText(
        q.right_answer
    )  # fill the first element of the list with the right answer, the rest with wrong ones
...
def next_question():
    ...
    print('Statistics\n-Total questions: ', window.total, '\n-Right answers: ',
          window.score)
    cur_question = randint(0,
                           len(questions_list) -
                           1)  # we don't need the old value,
...
btn_OK.clicked.connect(
    click_OK)  # by clicking on the button, we choose what exactly happens
